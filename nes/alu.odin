...
alu_read_u8_arg::proc (self: ^Alu6502, bus: ^addr.Bus, address: u16) -> u8 {
    data := addr.bus_read_u8(bus, address);
    self.regs.PC = address + 1;
    return data;
}

alu_read_u16_arg::proc (self: ^Alu6502, bus: ^addr.Bus, address: u16) -> u16 {
    data := addr.bus_read_u16(bus, address);
    self.regs.PC = address + 2;
    return data;
}

alu_pop::proc (self: ^Alu6502, bus: ^addr.Bus) -> u8{
...
alu_push::proc (self: ^Alu6502, bus: ^addr.Bus, data: u8){
...
alu_push_sr::proc (self: ^Alu6502, bus: ^addr.Bus) {
    alu_push(self, bus, self.regs.SR);
}
...
alu_step::proc (self: ^Alu6502, bus: ^addr.Bus) -> Alu6502_Result {
    ...
    decode_operation(self, opcode.ol, bus, &dest);
    return Alu6502_Result{cycles = opcode.cycles};
}

@(private)
decode_operand::proc (self: ^Alu6502, mode: AddressingLabel, bus: ^addr.Bus, opr: ^Operand) {
    switch(mode){
        case .ABS:
...
@(private)
decode_operation::proc (self: ^Alu6502, op: OpcodeLabel, bus: ^addr.Bus, opr: ^Operand) {

}


load::proc (src: ^Operand, dest: ^Operand, alu: ^Alu6502, bus: ^addr.Bus) {
...
transfer::proc (src: ^Operand, dest: ^Operand, alu: ^Alu6502) {
    srcaddr, ok1 := src.(^u8); assert(ok1);
    destaddrr, ok2 := dest.(^u8); assert(ok2);
    result := srcaddr^;
    destaddrr^ = result;
...
arithmetic::proc (src: ^Operand, alu: ^Alu6502, bus: ^addr.Bus) {
    data:u8;
...
compare::proc (src: ^Operand, dest: ^Operand, alu: ^Alu6502, bus: ^addr.Bus) {
    memory:u8;
    switch choice in src {
        case u8:
            memory = choice;
...
branch::proc (src: ^Operand, alu: ^Alu6502, flag: Flags, cond: bool) {
...
inc_or_dec::proc (src: ^Operand, alu: ^Alu6502, option: enum(int){INC=1,DEC=-1}, bus: ^addr.Bus) {
    result:int;
    switch eaddr in src {
        case u16:
...
shift::proc (src: ^Operand, dir: enum{RIGHT,LEFT}, alu: ^Alu6502, bus: ^addr.Bus) {
    carry:bool;
    result:u8;
    data:u8;
    switch eaddr in src {
...
rotate::proc (src: ^Operand, dir: enum{RIGHT,LEFT}, alu: ^Alu6502, bus: ^addr.Bus) {
...
bitwise::proc (src: ^Operand, alu: ^Alu6502, op: BitwiseOp, bus: ^addr.Bus) {
    data, result:u8;
    switch eaddr in src {
        case u8:
            data = eaddr
...
jump::proc (src: ^Operand, alu: ^Alu6502, op: JumpOp, bus: ^addr.Bus) {
